...
public:
  ...
  testbench(sc_module_name name) : testbench_vbase(name) {
    ...
    // 分支预测器监测器
    m_bp_monitor = new BPMonitor(m_dut->m_rtl);
  }
  //-----------------------------------------------------------------
  // Trace
  //-----------------------------------------------------------------
  void add_trace(std::string vcd_filename) {
    if (!waves_enabled())
      return;
  ...
  //-----------------------------------------------------------------
  void write(uint32_t addr, uint8_t data) { m_dcache_mem->write(addr, data); }
  //-----------------------------------------------------------------
  // write: Read byte from memory
  //-----------------------------------------------------------------
  uint8_t read(uint32_t addr) { return m_dcache_mem->read(addr); }
};
