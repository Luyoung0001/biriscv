...
public:
    ...
    testbench_vbase(sc_module_name name): sc_module(name), m_verilate_vcd(NULL)
    {
        SC_CTHREAD(process, clk);
        SC_CTHREAD(monitor, clk);
        SC_METHOD(dump_method);
        sensitive << clk;
    }

    void dump_method(void)
    {
        dump_trace();
    }
    ...
    void dump_trace(void)
    {
        if (m_verilate_vcd)
        {
            m_verilate_vcd->dump(sc_time_stamp().to_double());
        }
    }
