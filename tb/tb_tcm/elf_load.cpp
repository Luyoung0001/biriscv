...
bool elf_load::load(void) {
  ...
  while ((scn = elf_getscn(e, section_idx)) != NULL) {
    ...
    if (!shdr) {
      ...
      // 在段处理循环中修改打印
      printf("Loading Section [%s]: 0x%08x - 0x%08x (%d bytes)\n",
             elf_strptr(e, shstrndx, shdr->sh_name), shdr->sh_addr,
             shdr->sh_addr + shdr->sh_size - 1, shdr->sh_size);
      ...
      if ((shdr64->sh_flags & SHF_ALLOC) && (shdr64->sh_size > 0)) {
        ...
        printf("Memory: 0x%lx - 0x%lx (Size=%ldKB) [%s]\n", shdr64->sh_addr,
               shdr64->sh_addr + shdr64->sh_size - 1, shdr64->sh_size / 1024,
               elf_strptr(e, shstrndx, shdr64->sh_name));
    ...
    // 32-bit target - section which need allocating
      ...
      printf("Memory: 0x%x - 0x%x (Size=%dKB) [%s]\n", shdr->sh_addr,
             shdr->sh_addr + shdr->sh_size - 1, shdr->sh_size / 1024,
