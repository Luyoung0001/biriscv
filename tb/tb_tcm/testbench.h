...
public:
  ...
  void process(void) {
    ...
    int option_index = 0;
    while ((c = getopt_long(m_argc, m_argv, GETOPTS_ARGS, long_options,
                            &option_index)) != -1) {
      switch (c) {
      case 'f':
        filename = optarg;
  ...
  void set_argcv(int argc, char *argv[]) {
    m_argc = argc;
    m_argv = argv;
  }
  ...
  void add_trace(sc_trace_file *fp, std::string prefix) {
    ...
    // Enable Verilator trace (must be called after sc_start(SC_ZERO_TIME))
...
#define TRACE_SIGNAL(a) sc_trace(fp, a, #a);
  ...
  void write(uint32_t addr, uint8_t data) {
    // m_dut->m_rtl->vlSymsp->TOP__v__u_tcm.write(addr, data);
    m_dut->m_rtl->rootp->v->u_tcm->write(addr, data);
  }
  //-----------------------------------------------------------------
  // write: Read byte from memory
  //-----------------------------------------------------------------
  uint8_t read(uint32_t addr) {
    // return m_dut->m_rtl->vlSymsp->TOP__v__u_tcm.read(addr);
    m_dut->m_rtl->rootp->v->u_tcm->read(addr);
  }
};
